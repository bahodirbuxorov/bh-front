...
import { CashFlowForecast } from './tabs/CashFlowForecast';
import { BankReconciliation } from './tabs/BankReconciliation';
import { BudgetVsActual } from './tabs/BudgetVsActual';

export const CashBankPage: React.FC = () => {
    const { t } = useLanguageStore();
    const { addToast } = useUIStore();
    const [activeTab, setActiveTab] = useState<'transactions' | 'forecast' | 'reconciliation' | 'budget'>('transactions');
        ...
        <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex-wrap">
                {[
                    ...
                    { key: 'forecast', label: 'Pul Oqimi Prognozi' },
                    { key: 'reconciliation', label: 'Bank Moslashtirish' },
                    { key: 'budget', label: 'Byudjet vs Haqiqiy' },
                ].map(({ key, label }) => (
            ...
            {activeTab === 'budget' && <BudgetVsActual />}
